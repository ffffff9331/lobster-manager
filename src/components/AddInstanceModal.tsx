        ...
        <div className="modal-body">
          ...
          >
            <div style={{ display: "flex", justifyContent: "space-between", gap: 12, alignItems: "center" }}>
              <div>
                <div style={{ fontWeight: 600 }}>本机 / WSL2 实例检测</div>
                <div style={{ fontSize: 13, color: "var(--text-secondary)", marginTop: 4 }}>
                  先探测本机 `127.0.0.1:18789`；Windows 下若未命中，会继续检测 WSL2 内的 OpenClaw。检测到后可直接加入，也可以手动填写自定义 WSL2 地址。
                </div>
              </div>
              <button className="btn btn-secondary" onClick={() => void onDetectLocal()} disabled={detectingLocal}>
                {detectingLocal ? "检测中..." : "检测本机 / WSL2"}
              </button>
            </div>
            {localInstanceStatus ? (
              <div style={{ marginTop: 10, fontSize: 13, color: "var(--text-secondary)", display: "grid", gap: 4 }}>
                ...
                {localInstanceStatus.exists && localInstanceStatus.running ? (
                  <div style={{ marginTop: 6 }}>
                    <button className="btn btn-primary" onClick={onAddDetectedLocal}>{localInstanceStatus.type === "wsl" ? "加入 WSL2 实例" : "加入本机实例"}</button>
                  </div>
                ) : null}
              </div>
            ) : null}
          ...
          <div className="form-group">
            ...
            <small>
              {form.type === "local"
                ...
                : form.type === "wsl"
                  ? "WSL2 实例允许自定义可访问地址；命令仍通过 wsl.exe 执行。若走宿主转发，可填写 http://127.0.0.1:18789/。"
                  : form.type === "docker"
